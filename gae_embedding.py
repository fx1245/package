...
from gae.model import GATModelVAE
...
def main(raw_args=None):
    ...
    parser.add_argument('--GAEh_dim', type=int, default=512, help='Number of units in hidden layer 1.')
    parser.add_argument('--GAEz_dim', type=int, default=50, help='Number of units in hidden layer 2.')
    ...
    parser.add_argument('--GAEheads', default=3, type=int, help='number of clusters, 7 for cora, 6 for citeseer')
    parser.add_argument('--layerType', type=str, default='GATConv',
                        help='Single Precision/Double precision: Float/Double (default:Float)')
...
def GAEembedding(z, adj, args):
    ...
    if args.GAEmodel == 'gcn_vae':
        layer = 'GATConv'
        heads = 5
        model = GATModelVAE(feat_dim, args.GAEh_dim, args.GAEz_dim, args.GAEdropout, layerType=layer, heads=heads)
    # else:
